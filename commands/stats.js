...
module.exports = {
    ...
    async execute(message, args) {
        ...
        var summonerData = await league.Summoner.gettingByName(name).catch(
            (err) => {
                'use strict';
                console.error(err);
        ...
        var masteryData = await league.ChampionMastery.gettingBySummoner(
            summonerData.id,
            'NA1'
        );
        for (var j = 0; j < 10; j++) {
            ...
            data.push(
                `${j + 1} ${champ} ${mastery.championPoints} ${
